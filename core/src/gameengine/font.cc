...
void Font::RenderGlyph(char glyph, GamePoint position, float alpha, float angle) {
  if (glyph == ' ') {
    return;
  }
#if DEBUG
...
#endif
  glyphs_[glyph].Draw(game_engine_.game_point_to_screen_point(position), alpha, 1, angle, false);
}
